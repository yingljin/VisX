...
data_check <- function(df){
  ...
  empty_col <- apply(df, 2, function(x){sum(!is.na(x))})
  empty_name <- colnames(df)[empty_col==0]
  if(length(empty_name)>0){
    empty_name <- paste(empty_name, collapse = ", ")
    mes = paste("Warning:", empty_name, "missing for all observations")
    warning(mes)
  }
  ...
  if(length(identical_name)>0){
    ...
    mes = paste("Warning:", identical_name, "with same value for all observations")
    warning(mes)
  }
}
